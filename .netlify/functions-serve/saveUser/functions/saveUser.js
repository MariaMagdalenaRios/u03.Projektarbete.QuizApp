...
// functions/utils/supabaseClient.js
...
async function handler(event, context) {
  try {
    ...
    const { data: existing, error: getError } = await supabase.from("Users").select("*").eq("email", email).maybeSingle();
    if (getError) {
    ...
    if (existing) {
      ...
      const { data, error } = await supabase.from("Users").update(updatedUser).eq("email", email).select();
      if (error) {
